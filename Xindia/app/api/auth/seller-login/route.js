...
if (!process.env.NEXT_PUBLIC_API_URL) {
  console.error(
    '[seller-login] NEXT_PUBLIC_API_URL is not set — falling back to http://localhost:5000. ' +
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      console.error(`[seller-login] Backend at ${API_URL} unreachable or returned a non-JSON response:`, netErr.message);
